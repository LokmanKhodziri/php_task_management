            ...
            <form method="POST" action="{{ route('tasks.update', $task) }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full border rounded px-3 py-2 text-sm">{{ old('description', $task->description) }}</textarea>
                </div>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700">Update Task</button>
                    <a href="{{ route('projects.show', $task->project) }}" class="text-sm text-gray-500 hover:underline self-center">Cancel</a>
                </div>
            </form>
        </div>
    </div>
